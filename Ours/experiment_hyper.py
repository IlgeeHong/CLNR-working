...
parser.add_argument('--n_experiments', type=int, default=6)
...
parser.add_argument('--result_file', type=str, default="/results/SelfGCon_node_classification_")
...
for exp in range(args.n_experiments): 
    ...
    for _ in range(10):
        ...
        # results += [[args.model, args.dataset, args.epochs, args.n_layers, args.tau, args.lr1, args.lr2, args.wd2, args.channels, args.drop_rate_edge, args.drop_rate_feat, eval_acc]]
        ...
        acc += [eval_acc]
    acc_tensor = torch.FloatTensor(acc)
    results += [[out_dim, acc_tensor.mean()]]
    res1 = pd.DataFrame(results, columns = ['dim','acc'])
    res1.to_csv(file_path + "_" + args.dataset +  ".csv", index=False)
