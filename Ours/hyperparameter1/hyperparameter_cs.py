...
parser.add_argument('--result_file', type=str, default="/Ours/hyperparameter1/results/Hyperparameter")
...
def train_semi(model, data, fmr, edr, num_class, train_idx, k=None):
    model.train()
    optimizer.zero_grad()
                        ...
                        for wd2 in [1e-2, 1e-4]:
                            for epochs in [20, 50]:
                                ...
                                for exp in range(args.n_experiments): 
                                    ...
                                    ''' Model Pretraining '''
                                    model = SemiGCon(in_dim, hid_dim, out_dim, n_layers, tau, use_mlp=mlp_use)
                                    model = model.to(device)
                                    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr1, weight_decay=args.wd1)                       
                                    for epoch in range(epochs):
                                        loss = train_semi(model, data, fmr, edr, num_class, train_idx)
