...
parser.add_argument('--result_file', type=str, default="/Ours/results/LAST2")
...
def train(model, data, fmr, edr, proj, k=None):
    model.train()
    optimizer.zero_grad()
    ...
    loss = model.loss(z1, z2, layer=proj, k=k)
    loss.backward()
    optimizer.step()
    return loss.item()

results =[]
# for epochs in [2000, 4000, 6000, 8000, 10000]:
#     eval_acc_list = []
for exp in range(args.n_experiments):
    ...
    for epoch in range(args.epochs):
        loss = train(model, data, args.fmr, args.edr, args.proj, k=1024)
