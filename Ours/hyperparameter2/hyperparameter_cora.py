...
parser.add_argument('--result_file', type=str, default="/Ours/hyperparameter2/results/Hyperparameter2")
                ...
                for epochs in [20, 50]:
                    for wd1 in [0.0, 1e-5]:
                        ...
                        for exp in range(args.n_experiments): 
                            ...
                            ''' Model Pretraining '''
                            model = CLGR(in_dim, hid_dim, out_dim, n_layers, args.tau, use_mlp=args.mlp_use)
                            model = model.to(device)
                            optimizer = torch.optim.Adam(model.parameters(), lr=args.lr1, weight_decay=wd1)                       
                        ...
                        results += [[args.model, args.dataset, epochs, n_layers, args.tau, args.lr1, wd1, args.lr2, args.wd2, channels, fmr, edr, args.mlp_use, best_val_acc_mean]]
                        res1 = pd.DataFrame(results, columns=['model', 'dataset', 'epochs', 'layers', 'tau', 'lr1', 'wd1', 'lr2', 'wd2', 'channels', 'feat_mask_rate', 'edge_drop_rate', 'mlp_use', 'val_acc'])
